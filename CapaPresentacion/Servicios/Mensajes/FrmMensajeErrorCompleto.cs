...
namespace CapaPresentacion
{
    public partial class FrmMensajeErrorCompleto : Form
    {
        ...
        private void BtnEnviar_Click(object sender, EventArgs e)
        {
            ...
            {
                string rpta = 
                    EnviarEmailErrores.SendEmailError(this.Detalle_informacion, this.MetodoError, this.FormularioError, this.Informacion_corta);
                if (rpta.Equals("OK"))
                {
                    Mensajes.MensajeOkForm("Se envío el mensaje con el error correctamente");
                    this.Close();
                }
                else
                {
            ...
            {
                Mensajes.MensajeErrorForm("Hubo un error al enviar el correo electrónico con el error: " + ex.Message);
            }
        }
